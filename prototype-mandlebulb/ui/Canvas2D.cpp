...
void Canvas2D::renderImage(CS123SceneCameraData*, int width, int height) {
   ...
   // auto iTime = 4200;
    auto rayOrigin = glm::vec4{ 0, 3, -3, 1 };
    auto focalLength = 2.;
    ...
    Lights[0].dir = -look;//glm::vec4{ glm::normalize(glm::vec3{ 1, 2, 3 }), 0 };
    ...
    std::get<0>(ObjectRecords[0]) = [](auto&& pp) { 
        ...
        auto sdMandelbulb = [](auto&& pos) {
            ...
            for (int i = 0; i < 255; i++) {
                ...
                dr = std::pow(r, 8.) * 8. * dr + 1.0;

                // scale and rotate the point
                float zr = std::pow(r, 8.);
                theta = theta * 8.;
                phi = phi * 8.;
    ...
    std::get<1>(ObjectRecords[0]).cDiffuse = glm::vec4{ 1, 1, 1, 1 };
